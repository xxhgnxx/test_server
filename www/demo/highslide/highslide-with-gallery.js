	...
	hs.Expander.prototype = {
		...
		show: function () {
			...
			this.changeSize(
				1, {
					wrapper: {
						width: x.get('wsize') * 1.5,
						height: y.get('wsize') * 1.5,
						left: x.pos,
						top: y.pos
					},
					content: {
						left: x.p1 + x.get('imgPad') - x.size / 4,
						top: y.p1 + y.get('imgPad') - y.size / 4,
						// width:x.imgSize ||x.size,
						width: x.imgSize || x.size * 1.5,
						// height:y.imgSize ||y.size
						height: y.imgSize || y.size * 1.5
					}
				},
				hs.expandDuration
			);
		...
		sizeOverlayBox: function (doWrapper, doPanels) {
			...
			hs.setStyles(overlayBox, {
				width: x.size*1.5 + 'px',
				height: y.size*1.5 + 'px',
				"margin-left": x.size*(-0.25) + 'px',
				"margin-top": y.size*(-0.25)+ 'px',
			});
